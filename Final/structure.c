...
picross Initialisation (char* nom)
{
    picross picross1;
...
picross copierPicross(picross source, char* nom)
{
    int i,j;
    picross copie=Initialisation(nom);//Affectation mémoire.
...
int generateurHypo(picross test, picross *source, int *compt, char* nom)
{
    int i, j, check, hypo;
    *compt=*compt+1;
    if (*compt>1)
    ...
    else
    {
        ...
        hypo=generateurHypo(test1, source, compt, nom);
        if (hypo!=1)
        {
            free(test1.grille);
            test1=copierPicross(test, nom);
            test1.grille[i][j]=blanc;
            return generateurHypo(test1, source, compt, nom);
        }
        else
        {
            return 1;
...
void resolveur(char* nom)
{

    picross picross1;
    picross pictest;
    int compt=0;
    picross1=Initialisation(nom);

    JobList jobl1;
    jobl1=initJobList(picross1);
    resolution(jobl1, picross1);
    if (picross1.transpose%2==1) transposer(picross1);
    printf("Grille resolue avec les regles : \n");
    afficherGrille(picross1);

    pictest=copierPicross(picross1, nom);
    generateurHypo(pictest,&picross1,&compt, nom);
    if (picross1.transpose%2==1) transposer(picross1);

    printf("\nGrille resolue avec le generateur d'hypotheses: \n");
    afficherGrille(picross1);
    printf("\nIl y a eu %i appels recursifs.\n",compt);
}
